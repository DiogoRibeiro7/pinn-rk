...
class RkPinnLoss(nn.Module):
    ...
    def __init__(
        ...
        if cfg.spatial_sampler is None:
            self.cfg.spatial_sampler = self._default_uniform_sampler  # type: ignore[assignment]
    ...
    def forward(self) -> Tensor:
        ...
        for n in range(times.steps.numel()):
            k_n = times.steps[n]
            x = self.cfg.spatial_sampler(self.cfg.n_x_train, device=device)  # [B,1]
            ...
            F  = torch.stack(f_stage,  dim=1) # [B,q,1]

            # Interpolant û and its time derivative at collocation nodes
            t_eval = t_stage.view(1, q).repeat(self.cfg.n_x_train, 1)  # [B,q]
            L_eval = lagrange_eval(t_eval, t_stage, w_nodes)            # [B,q,q]
            u_hat_eval = torch.einsum("bij, bj1 -> bi1", L_eval, U)     # [B,q,1]

            # Approximate û_t via symmetric finite differences around each stage time
            eps = 1e-6 * float(k_n)
            u_t_eval: List[Tensor] = []
            for i in range(q):
                ti = t_stage[i].item()
                t_left  = torch.full_like(x, ti - eps)
                t_right = torch.full_like(x, ti + eps)
                ui_r = self._eval_model(x, t_right)
                ui_l = self._eval_model(x, t_left)
                u_t_eval.append((ui_r - ui_l) / (2.0 * eps))
            u_t_eval = torch.stack(u_t_eval, dim=1)  # [B,q,1]
        ...
        if self.cfg.init_data is not None:
            ...
            grad_u = torch.autograd.grad(
                u_init, x0, grad_outputs=torch.ones_like(u_init),
                create_graph=True, retain_graph=True, only_inputs=True
            )[0]
            grad_u0 = torch.autograd.grad(
                u0, x0, grad_outputs=torch.ones_like(u0),
                create_graph=True, retain_graph=True, only_inputs=True
            )[0]
            total = total + F.mse_loss(grad_u, grad_u0)
