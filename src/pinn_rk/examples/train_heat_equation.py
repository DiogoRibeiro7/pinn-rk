...
def train_heat_equation(
    ...
    cfg = RkPinnConfig(
        tableau=bt, time_mesh=mesh, n_x_train=n_x_train,
        device=device, dtype=torch.float64, init_data=(x0, u0)
    )
...
def l2_error(model: nn.Module, T: float, nx: int = 1001, device: torch.device = torch.device("cpu")) -> float:
